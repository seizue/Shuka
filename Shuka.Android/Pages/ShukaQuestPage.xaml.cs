...
/// <summary>
/// Shuka Quest - Full-screen WebView browser with search features.
...
/// Includes in-page search functionality.
/// </summary>
public partial class ShukaQuestPage : ContentPage
{
    ...
    private readonly Stack<string> _backNavigationHistory = new();
    ...
    public ShukaQuestPage(string startUrl)
    {
        try
        {
            // Assign unique instance ID
            _instanceId = System.Threading.Interlocked.Increment(ref _instanceCounter);
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Creating instance #{_instanceId}");
            ...
            try
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Found existing NameScope, clearing it");
            ...
            catch (ArgumentException ex) when (ex.Message.Contains("already exists in this NameScope"))
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] NameScope conflict detected, attempting recovery");
            ...
            if (string.IsNullOrWhiteSpace(startUrl))
            {
                ...
                System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] Warning: Empty startUrl, using fallback");
            }
            ...
            _ = Task.Run(async () =>
            {
                ...
                await MainThread.InvokeOnMainThreadAsync(() =>
                {
                    if (InitialLoadingOverlay?.IsVisible == true)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] Initial loading overlay hidden by safety timeout");
                    }
                    ...
                    if (UrlLoadingIndicator?.IsVisible == true)
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] URL loading indicator hidden by safety timeout");
                    }
                });
            });
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Constructor error: {ex.Message}");
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Stack trace: {ex.StackTrace}");

            // Log to crash file
            try
            {
                var logPath = Path.Combine(FileSystem.CacheDirectory, "crash.log");
                var logEntry = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] ShukaQuestPage constructor: {ex.Message}\n{ex.StackTrace}\n\n";
    ...
    private void CleanupWebView()
    {
        ...
        try
        {
            if (SiteWebView != null)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Error clearing WebView source: {ex.Message}");
                }
                ...
                if (SiteWebView.Handler?.PlatformView is global::Android.Webkit.WebView androidWebView)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Error during Android WebView cleanup: {ex.Message}");
                    }
                }
#endif
            }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Error during WebView cleanup: {ex.Message}");
        }
    }
    ...
    private void Navigate(string url)
    {
        try
        {
            ...
            MainThread.BeginInvokeOnMainThread(() =>
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] UI update error: {ex.Message}");
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Navigate error: {ex.Message}");
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Stack trace: {ex.StackTrace}");
    ...
    private async void OnBackTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Back navigation error: {ex.Message}");
        }
    }
    ...
    private void OnForwardTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Forward navigation error: {ex.Message}");
        }
    }
    ...
    private void OnReloadTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Reload error: {ex.Message}");
    ...
    private async void OnTabsTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Tabs menu error: {ex.Message}");
        }
    }
    ...
    private async void OnAdBlockerToggleTapped(object? sender = null, TappedEventArgs? e = null)
    {
        ...
        await MainThread.InvokeOnMainThreadAsync(() =>
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Ad blocker toggle refresh: {ex.Message}");
            }
        });
    }
    ...
    private async void OnTranslateTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Translate error: {ex.Message}");
    ...
    private async Task<string?> PickBestLeaveTranslateUrlAsync()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] PickBestLeaveTranslateUrl tracker: {ex.Message}");
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] PickBestLeaveTranslateUrl dom: {ex.Message}");
        }
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] PickBestLeaveTranslateUrl top href: {ex.Message}");
        }
    ...
    private string? GetAddressBarUrlForLeave()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] GetAddressBarUrlForLeave label: {ex.Message}");
        }
...
""";
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] AddLeaveCandidatesFromTranslateDom parse: {ex.Message}");
        }
    }
        ...
        await MainThread.InvokeOnMainThreadAsync(() =>
        {
            try
            {
                ...
                if (SiteWebView.Handler?.PlatformView is global::Android.Webkit.WebView wv)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] DontTranslate ReloadWeb LoadUrl: {ex.Message}");
                    }
                }
#endif
                SiteWebView.Source = new UrlWebViewSource { Url = url };
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] DontTranslateReload UI: {ex.Message}");
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                await MainThread.InvokeOnMainThreadAsync(async () =>
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] ScheduleTranslateEmbeddedOriginalSync: {ex.Message}");
                    }
                });
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] ScheduleTranslateEmbeddedOriginalSync outer: {ex.Message}");
            }
        });
    }
    ...
    private void OnNavigating(object sender, WebNavigatingEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] OnNavigating error: {ex.Message}");
            e.Cancel = true;
    ...
    private async void OnNavigated(object sender, WebNavigatedEventArgs e)
    {
        try
        {
            ...
            // Inject Chinese site compatibility for JJWXC and similar sites
            _ = InjectChineseSiteCompatibilityAsync();
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] OnNavigated error: {ex.Message}");
            _isLoading = false;
            LoadingBar.IsVisible = false;
    ...
    private async void OnCopyUrlTapped(object sender, TappedEventArgs e)
    {
        try
        {
            string urlToCopy = string.IsNullOrWhiteSpace(_currentUrl) ? _actualWebNavigationUrl : _currentUrl;
            if (!string.IsNullOrWhiteSpace(urlToCopy))
            {
                await Clipboard.Default.SetTextAsync(urlToCopy);
                await ShowQueuedToastAsync("URL copied!");
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Copy URL error: {ex.Message}");
        }
    }

    /// <summary>
    /// Configures WebView for proper image handling and context menu support.
    /// Also handles link long-press with preview and context menu.
    /// </summary>
    private void ConfigureWebViewForImageHandling()
    {
        try
        {
            ...
            if (SiteWebView.Handler?.PlatformView is global::Android.Webkit.WebView androidWebView)
            {
                // Enable proper user agent for better compatibility with JJWXC and other Chinese sites
                ...
                // Enable proper encoding support for Chinese sites (JJWXC uses GBK)
                settings.DefaultTextEncodingName = "UTF-8";
                settings.MinimumFontSize = 8;
                settings.MinimumLogicalFontSize = 8;

                // Enable zoom and text reflow for better reading
                settings.SetSupportZoom(true);
                settings.BuiltInZoomControls = true;
                settings.DisplayZoomControls = false;
                settings.TextZoom = 100;

                // Enable wide viewport and proper rendering
                settings.UseWideViewPort = true;
                settings.LoadWithOverviewMode = true;

                // Disable autoplay windows
                ...
                // Hook long-press context actions for both images and links.
                // Fix: context menu actions inside the Mini Browser not functioning.
                //
                // Design notes:
                ...
                //  • AnchorType covers <a> tags with text links.
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] ConfigureWebViewForImageHandling error: {ex.Message}");
        }
    }
    ...
    /// <summary>
    /// Configures WebView for Chinese site compatibility (JJWXC, etc.).
    /// </summary>
    private void ConfigureWebViewForChineseSites()
    {
        // This will be called after page load to inject anti-bot and encoding fixes
    }

    /// <summary>
    /// Injects JavaScript for Chinese site compatibility after page loads.
    /// </summary>
    private async Task InjectChineseSiteCompatibilityAsync()
    {
        try
        {
            if (string.IsNullOrWhiteSpace(_currentUrl))
            ...
            // Only inject for Chinese novel sites
            if (!host.Contains("jjwxc") && !host.Contains("69shuba") && 
                !host.Contains("czbooks") && !host.Contains("52shuku"))
                return;

            string script = @"
                (function() {
                    // Fix character encoding issues
                    if (document.characterSet && document.characterSet.toLowerCase().indexOf('gb') !== -1) {
                        document.charset = 'utf-8';
                    }
                    
                    // Anti-bot detection bypass - simulate human behavior
                    const simulateHuman = () => {
                    ...
                    const interval = setInterval(simulateHuman, 3000 + Math.random() * 2000);
                    
                    // Clean up after 30 seconds
                    setTimeout(() => clearInterval(interval), 30000);
                    
                    // Override common bot detection
                    ...
                    console.log('Shuka Quest: Chinese site compatibility injected');
                })();
            ";

            await SiteWebView.EvaluateJavaScriptAsync(script);
            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] Chinese site compatibility injected");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] InjectChineseSiteCompatibility error: {ex.Message}");
        }
    }
    ...
    private async Task SearchInPageAsync(string query)
    {
        try
        {
            ...
            // JavaScript to find and highlight all matches
            string script = $@"
                (function() {{
                    let query = '{JavaScriptStringEscape(query)}';
                    if (!query) return 0;

                    // Clear previous highlights
                    document.querySelectorAll('.shuka-quest-highlight').forEach(el => {{
                    ...
                    nodes.forEach(textNode => {{
                        let regex = new RegExp('({RegexEscape(query)})', 'gi');
                        ...
                        while ((match = regex.exec(textNode.textContent)) !== null) {{
                            ...
                            fragment.appendChild(document.createTextNode(
                                textNode.textContent.substring(lastIdx, match.index)
                            ));
                            // Highlighted match
                            let span = document.createElement('span');
                            span.className = 'shuka-quest-highlight';
                        ...
                        // Remaining text
                        fragment.appendChild(document.createTextNode(
                            textNode.textContent.substring(lastIdx)
                        ));
                        textNode.parentNode.replaceChild(fragment, textNode);
                    }});

                    // Scroll to first match
                    let firstMatch = document.querySelector('.shuka-quest-highlight');
                    if (firstMatch) {{
            ...
            string result = await SiteWebView.EvaluateJavaScriptAsync(script);
            if (int.TryParse(result, out int count))
            {
                if (count > 0)
                {
                    await ShowQueuedToastAsync($"{count} match{(count == 1 ? "" : "es")} found");
                }
                else
                {
                    await ShowQueuedToastAsync("No matches found");
                }
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Search error: {ex.Message}");
        }
    }
    ...
    private async void ClearSearchHighlights()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Clear search error: {ex.Message}");
        }
    }
    ...
    private async Task InjectAdBlockerAsync()
    {
        try
        {
            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] ===== AD BLOCKER INJECTION START =====");
...
";
            ...
            {
                System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] WARNING: Ad blocker script is empty!");
                return;
            }

            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Ad blocker script length: {js.Length} chars");

            // First pass — run immediately
            await SiteWebView.EvaluateJavaScriptAsync(js);
            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] ✓ Ad blocker pass 1 complete");
...
";
            ...
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] After pass 1: {result}");

            // Second pass — wait for lazy-loaded / script-injected ads (500ms)
            await Task.Delay(500);
            await SiteWebView.EvaluateJavaScriptAsync(js);
            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] ✓ Ad blocker pass 2 complete");

            // Third pass — catch delayed ads (1.5s)
            await Task.Delay(1000);
            await SiteWebView.EvaluateJavaScriptAsync(js);
            result = await SiteWebView.EvaluateJavaScriptAsync(inspectJs);
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] After pass 3: {result}");

            // Fourth pass — some sites inject ads even later (3s)
            await Task.Delay(1500);
            await SiteWebView.EvaluateJavaScriptAsync(js);
            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] ✓ Ad blocker pass 4 complete");

            // Fifth pass — final cleanup (5s)
            await Task.Delay(2000);
            await SiteWebView.EvaluateJavaScriptAsync(js);
            result = await SiteWebView.EvaluateJavaScriptAsync(inspectJs);
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] After pass 5: {result}");

            System.Diagnostics.Debug.WriteLine("[ShukaQuestPage] ===== AD BLOCKER INJECTION COMPLETE =====");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] ❌ AdBlocker inject error: {ex.Message}");
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Stack trace: {ex.StackTrace}");
        }
    }
    ...
    private void UpdateNavigationButtons()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] UpdateNavigationButtons error: {ex.Message}");
        }
    }
    ...
    private void ExecuteNavigationBarUpdate(string url)
    {
        ...
        _ = Task.Run(() =>
        {
            ...
            MainThread.BeginInvokeOnMainThread(() =>
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Navigation bar update error: {ex.Message}");
                }
            });
        });
    }
    ...
    private async void OnBookmarkTapped(object sender, TappedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Bookmark error: {ex.Message}");
    ...
    private async Task AddBookmarkAsync(string url, string siteName)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] AddBookmark error: {ex.Message}");
            await DisplayAlertAsync("Error",
    ...
    /// <summary>
    /// Auto-fill the Paste URL entry from the clipboard when the menu opens,
    /// but only if the clipboard text looks like a URL and the entry is empty.
    /// </summary>
    private async Task TryAutoFillMenuPasteUrlFromClipboardAsync()
    {
        try
        {
            if (MenuPasteUrlEntry == null) return;
            if (!string.IsNullOrWhiteSpace(MenuPasteUrlEntry.Text)) return;
            ...
            {
                MenuPasteUrlEntry.Text = text;
            }
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] Clipboard auto-fill error: {ex.Message}");
        }
    }
    ...
    private void OnMenuPasteUrlCompleted(object sender, EventArgs e)
        => _ = NavigateFromMenuPasteUrlAsync();

    private void OnMenuPasteUrlGoTapped(object sender, TappedEventArgs e)
        => _ = NavigateFromMenuPasteUrlAsync();

    private async Task NavigateFromMenuPasteUrlAsync()
    {
        try
        {
            string? text = MenuPasteUrlEntry?.Text?.Trim();
            if (string.IsNullOrWhiteSpace(text))
            {
                await ShowQueuedToastAsync("Enter a URL first");
                return;
            }

            // Normalize and validate URL before navigating.
            ...
            {
                await ShowQueuedToastAsync("Invalid URL");
                return;
            }

            await HideBrowserMenuAsync();
            Navigate(normalized);
            if (MenuPasteUrlEntry != null)
                MenuPasteUrlEntry.Text = string.Empty;
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[ShukaQuestPage] NavigateFromMenuPasteUrl error: {ex.Message}");
        }
    }
